...
#             ('fiscal_status', '=', 'normal'),
    ...
    def process_606_report_data(self, values):
        ...
        EXP_TYPE = str(
            values['service_type'] if values['service_type'] else "").strip()
        ...
        for rec in self:
            ...
            for inv in invoice_ids:
                ...
                values = {
                    ...
                    'service_type': inv.l10n_do_expense_type,#dict(inv._fields['l10n_do_expense_type'].get_description(
#                         self.env).get('selection')).get(inv.l10n_do_expense_type),
                    ...
                    'payment_type': inv.payment_form,#dict(inv._fields['payment_form'].get_description(
#                         self.env).get('selection')).get(inv.payment_form),  #inv.payment_form,
    ...
    def _convert_to_user_currency(self, base_currency, amount, date=fields.Date.today()):
        ...
        ctx = context.copy()
        return abs(base_currency_id.with_context(ctx)._convert(
            amount, user_currency_id, company_id, date))
